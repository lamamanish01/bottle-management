        ...
        <div class="table-responsive">
            <table class="table table-hover table-striped align-middle">
                ...
                <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="btn-group btn-group-sm" role="group">
                                    @can('view collectors')
                                        <a href="{{ route('collectors.show', $collector) }}" class="btn btn-outline-info" title="View"><i class="fas fa-eye"></i></a>
                                    @endcan
                                    @can('edit collectors')
                                        <a href="{{ route('collectors.edit', $collector) }}" class="btn btn-outline-warning" title="Edit"><i class="fas fa-edit"></i></a>
                                    @endcan
                                    @can('delete collectors')
                                        <form action="{{ route('collectors.destroy', $collector) }}" method="POST" style="display:inline-block;">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="btn btn-outline-danger" title="Delete" onclick="return confirm('Delete this collector?')">
                    ...
                    @empty
                        <tr><td colspan="6" class="text-center text-muted py-3">No collectors found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
