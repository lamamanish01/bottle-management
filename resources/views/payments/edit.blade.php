...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1><i class="fas fa-credit-card me-2"></i>Edit Payment #{{ $payment->id }}</h1>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                        <option value="incoming" {{ old('type', $payment->type) == 'incoming' ? 'selected' : '' }}>Incoming (from Buyer)</option>
                        <option value="outgoing" {{ old('type', $payment->type) == 'outgoing' ? 'selected' : '' }}>Outgoing (to Collector)</option>
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" class="form-control @error('payable_id') is-invalid @enderror" id="payable_id" name="payable_id" value="{{ old('payable_id', $payment->payable_id) }}" required>
                    @error('payable_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    <small class="text-muted">Sale or Collection ID.</small>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('reference') is-invalid @enderror" id="reference" name="reference" value="{{ old('reference', $payment->reference) }}" placeholder="Cheque #, Transfer ID, etc.">
